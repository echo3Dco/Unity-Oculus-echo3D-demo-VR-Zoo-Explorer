...
public class VideoechoAR : MonoBehaviour
{
    ...
    public void DownloadEntryAssets(Entry entry, string serverURL)
    {
        ...
        if (true)
        {
            ...
            // Handle video hologram
                ...
                string value = "";         
                string indexStr = (entry.getAdditionalData() != null && entry.getAdditionalData().TryGetValue("Index", out value)) ? value : "-1";
                SingleEchoSelection selection = this.GetComponent<SingleEchoSelection>();
                if (selection != null)
                {
                    if (!selection.isInList(indexStr))
                        return;
                }
                ...
                videoPlane.name = indexStr + " " + videoHologram.getFilename();
